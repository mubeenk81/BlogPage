        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-200 rounded shadow">
                <thead class="bg-gray-100 text-gray-700">
                    <tr>
                        <th scope="col" class="py-3 px-4 text-left text-sm font-medium">Post ID</th>
                        ...
                        <th scope="col" class="py-3 px-4 text-left text-sm font-medium">Created At</th>
                    ...
                    @foreach($posts as $post)
                        <tr>
                            ...
                            <td class="py-3 px-4 text-sm space-x-2">
                                <a href="{{ route('posts.show', $post->id) }}" class="text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-300" aria-label="View Post {{ $post->title }}">
                                    View
                                </a>

                                @if(auth()->check() && auth()->id() === $post->user_id)
                                    <a href="{{ route('posts.edit', $post->id) }}" class="text-green-600 hover:text-green-800 focus:outline-none focus:ring-2 focus:ring-green-300" aria-label="Edit Post {{ $post->title }}">
                                        Edit
                                    </a>
                                    ...
                                    <form action="{{ route('posts.destroy', $post->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this post?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800 focus:outline-none focus:ring-2 focus:ring-red-300" aria-label="Delete Post {{ $post->title }}">
                                            Delete
                                        </button>
                                    </form>
                                @endif
